...
class ConcatModel(nn.Module):
    def __init__(self,dp_mode,path):
        super().__init__()
...
def main2(self):
    ...
    os.makedirs('model_dict/seedPriConcat/'+self.path, exist_ok=True)
    ...
    whole_record_path = 'model_dict/seedPriConcat/' + self.path + 'whole_record.txt'
    best_record_path = 'model_dict/seedPriConcat/' + self.path + 'best_record.txt'
    save_model_path = 'model_dict/seedPriConcat/' + self.path + 'best_f1.pickle'
    ...
    for epoch in range(epochs):
        ...
        print("Time cost (total from training start):", time_now - time_start)
        with open(whole_record_path, "a") as file:
            file.write(record)
